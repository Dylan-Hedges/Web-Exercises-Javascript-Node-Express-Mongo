...
router.get("/campgrounds/:id/comments/:comment_id/edit", middleware.checkCommentOwnership, function(req, res){
    //Checks the campground exists
    Campground.findById(req.params.id, function(err, foundCampground){
        if(err || !foundCampground){
            req.flash("error", "No campground found.");
            //We use "return" with "res.redirect" if we dont have an else
            return res.redirect("back");
        }
        //Checks the comment exists
        //Finds the comment for the campground in the DB and either redirects back or renders the edit comment form, foundComment - store the comment text in the variable "foundComment" which is passes into "edit.ejs" under the name "comment"
